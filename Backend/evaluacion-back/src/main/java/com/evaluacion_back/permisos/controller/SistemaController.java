...
public class SistemaController {
    ...
    public Sistema crear(@RequestBody Sistema sistema) {
        if (sistema.getActivo() == null) {
            sistema.setActivo(true); // Inicializador por defecto si viene vacío
        }
    ...
    public Sistema actualizar(@PathVariable Integer id, @RequestBody Sistema sistemaDetalles) {
        ...
        if (sistemaDetalles.getDescripcion() != null) {
            sistema.setDescripcion(sistemaDetalles.getDescripcion());
        }
        
        return sistemaRepository.save(sistema);
    }

    //Endpoint para eliminar sistema 
    @DeleteMapping("/{id}")
